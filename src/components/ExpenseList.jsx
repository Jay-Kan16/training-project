...
export default function ExpenseList() {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <p className="text-sm text-gray-500 py-6 text-center">No expenses match these filters.</p>
      ) : (
            ...
            return (
              <li key={exp._id} className="py-3.5 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-sm font-bold text-gray-800">{formatCurrency(exp.amount)}</span>
                  <button
