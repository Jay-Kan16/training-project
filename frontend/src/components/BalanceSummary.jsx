...
export default function BalanceSummary() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="font-medium text-slate-100 mb-2">Balances</h3>
      ...
      <div>
        <h3 className="font-medium text-slate-100 mb-2">Suggested settlements</h3>
        {settlements.length === 0 ? (
          <p className="text-sm text-slate-400">Everyone is settled up 🎉</p>
        ) : (
          <ul className="space-y-1.5">
            {settlements.map((s, i) => (
              <li key={i} className="text-sm bg-primary-500/15 text-primary-200 px-3 py-2 rounded-lg">
